...
mod tests {
    ...
    impl SecretsBackend for TestSecretsBackend {
        ...
        fn delete(&self, _uri: &SecretUri) -> secrets_core::Result<secrets_core::SecretVersion> {
            Err(secrets_core::Error::Backend(
                "test backend is read-only".into(),
            ))
        }
        ...
        fn versions(
            &self,
            _uri: &SecretUri,
        ) -> secrets_core::Result<Vec<secrets_core::SecretVersion>> {
            Ok(vec![secrets_core::SecretVersion {
                version: 1,
                deleted: false,
            }])
        }
