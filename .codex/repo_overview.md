...
- **Location:** libs/core/src/prelude.rs:24  
  **Status:** TODO  
  **Short description:** Align `SecretsResolver` trait with greentic-secrets abstractions to avoid bespoke features.
- **Location:** conformance/webchat/tests/webchat_conformance.rs:67,93,99,107  
  **Status:** Stub (unimplemented)  
  **Short description:** Test secrets backend lacks write/list/delete/version support; only read is implemented for conformance harness.
- **Location:** libs/core/examples/run_standalone.rs:167,197,201,205  
  **Status:** Stub (unimplemented)  
  **Short description:** Static secrets backend in standalone WebChat example intentionally does not support write/list/delete/version operations.
- **Location:** libs/core/src/platforms/webchat/standalone.rs:1005,1031,1038,1045  
  **Status:** Stub (unimplemented)  
  **Short description:** Test secrets backend used in WebChat standalone tests omits write/list/delete/version operations.
- **Location:** libs/core/tests/webchat_support.rs:48,70,74,78  
  **Status:** Stub (unimplemented)  
  **Short description:** WebChat test secrets backend only supports read/exists; mutations and listing are unimplemented for fixtures.

## 4. Broken, Failing, or Conflicting Areas
- **Location:** Workspace tests (`cargo test --workspace --no-run`)  
  **Evidence:** Command blocked waiting for Cargo build directory lock; tests/build could not be executed in this run.  
  **Likely cause / nature of issue:** Another cargo process or stale lock prevented compiling tests, so current pass/fail status is unknown.

## 5. Notes for Future Work
- Resolve Cargo build directory lock and run `cargo test --workspace` to verify current health and adapter conformance suites.
- Consider fleshing out the WebChat test/standalone secrets backends if write/list/delete/version behavior is needed beyond fixtures.
- Revisit `SecretsResolver` TODO to align with greentic-secrets interfaces per design doc.
