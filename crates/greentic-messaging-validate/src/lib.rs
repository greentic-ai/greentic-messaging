...
use greentic_types::{PROVIDER_EXTENSION_ID, PackManifest, ProviderDecl};
use serde_json::Value;

pub fn messaging_validators() -> Vec<Box<dyn PackValidator>> {
    vec![
        Box::new(MessagingProviderDeclValidator),
        Box::new(MessagingSetupFlowContractValidator),
        Box::new(MessagingSubscriptionsContractValidator),
        Box::new(MessagingSecretRequirementsPresenceValidator),
    ]
}
...
impl PackValidator for MessagingProviderDeclValidator {
    fn id(&self) -> &'static str {
        "messaging-provider-decls"
    }

    fn applies(&self, manifest: &PackManifest) -> bool {
        is_messaging_pack(manifest)
    }
...
struct MessagingSetupFlowContractValidator;

impl PackValidator for MessagingSetupFlowContractValidator {
    fn id(&self) -> &'static str {
        "messaging-setup-flow"
    }

    fn applies(&self, manifest: &PackManifest) -> bool {
        is_messaging_pack(manifest) && has_setup_declaration(manifest)
    }
    ...
    fn validate(&self, manifest: &PackManifest) -> Vec<Diagnostic> {
        ...
        if !has_public_url {
            diagnostics.push(diagnostic(
                ...
                Some(
                    "Expose PUBLIC_BASE_URL (or equivalent) in the provider config schema."
                        .to_owned(),
                ),
            ));
        }
...
struct MessagingSubscriptionsContractValidator;

impl PackValidator for MessagingSubscriptionsContractValidator {
    fn id(&self) -> &'static str {
        "messaging-subscriptions-flow"
    }

    fn applies(&self, manifest: &PackManifest) -> bool {
        is_messaging_pack(manifest) && has_subscriptions_declaration(manifest)
    }
...
struct MessagingSecretRequirementsPresenceValidator;

impl PackValidator for MessagingSecretRequirementsPresenceValidator {
    fn id(&self) -> &'static str {
        "messaging-secret-requirements"
    }
    ...
    fn applies(&self, manifest: &PackManifest) -> bool {
        ...
        let providers = provider_decls(manifest);
        providers.iter().any(|provider| !provider.ops.is_empty())
    }
